...
https://zhuanlan.zhihu.com/p/18120106739
...
复制到Edge或者Chrome打开（这步别忘了连上网）

Fn+F12或右键检查打开网络请求界面，筛选器选媒体，随便找一个名称里带screen的mp4文件请求复制（**选cUrl+cmd**）

新建一个文本文档以英文重命名并修改后缀名为***.cmd\***，右键编辑，粘贴刚刚复制的代码

在最后一行追加 -o 输出文件.mp4

并用Ctrl+F查找**range**一行（不是if-range）并删除（这样是为了去除切片模式以获取完整视频）
...
使用vscode搭配github copilot编程，感觉优势在于每次问询都是按照次数（premium request），这样避免token的巨量消耗，且价格合理，1x模型问一次花费0.04USD。但据说现在最好是claude code的编程方式，没用过

目前的工作流是搭配传统的linus prompt，gemini3.5flash的plan模式总览项目，执行后用gpt5.2codex修复编译报错

1. 传统linus prompt

   https://caelum.moe/articles/claude-code-linus-prompt
   
   更新改进后的linus prompt但是claudecode
   
   https://github.com/kingkongshot/Pensieve
   
   放到vscode项目目录下的AGENTS.md

2. 感觉claude写代码很冲动（但是快），幻觉问题较为突出，有时自己臆想。gemini就比较温和理智（但是思考过程有点慢），gpt5.2codex对于vscode的各功能集成优势大，在解决报错只需要#promblem就识别IDE标红的地方

## 光猫改桥接

https://www.cnblogs.com/kasusa/p/17752927.html

## 关闭Windows defender防止下载的破解软件被抓取

以HEU_KMS_Activator激活windows为例https://github.com/zbezj/HEU_KMS_Activator/releases
